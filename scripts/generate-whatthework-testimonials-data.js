...
async function generateWhatTheWorkTestimonialsData() {
  try {
    ...
    console.error('Error generating whatthework testimonials data:', error);
    if (error.code === 'ENOENT' && error.path === testimonialsDir) {
      console.warn('WhatTheWork testimonials directory does not exist. Skipping testimonials generation.');
      await fs.ensureDir(outputDir);
